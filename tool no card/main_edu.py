...
from modules.Bot_edu import BotManagerEdu

class WorkerThreadEdu(QThread):
    finished = pyqtSignal()
    ...
    def run(self):
        ...
        # 3. Chạy tạo account edu
        self.run_create_edu_account(data_file)
        self.finished.emit()
    ...
    def run_create_edu_account(self, data_file):
        # Khởi tạo Bot Manager cho Edu
        self.bot_instance = BotManagerEdu(
            token=self.config['token'],
            num_threads=self.config['threads'],
            chromedriver=self.config['driver'],
            headless_mode=self.config['headless_mode']
        )
        bot_manager = self.bot_instance
        success_count = 0
        ...
        for i in range(1, self.config['tries'] + 1):
            ...
            try:
                self.add_log(f"🔄 Đang tạo tài khoản edu #{i}...")
                results = bot_manager.start()
                
                if not results:
                    self.add_log(f"❌ Error creating edu account #{i}")
                    continue

                # Lưu tất cả các account đã tạo
                for result in results:
                    self.add_log(f"✅ Created edu account: {result['email']}")
                    # Lưu account vào file
                    with open(data_file, 'a', encoding='utf-8') as f:
                        f.write(f"{result['email']}:{result['password']}\n")
                    success_count += 1
                
                self.add_log(f"🚩 Created {success_count}/{self.config['tries']} edu accounts")
...
class MainWindowEdu(QMainWindow):
    def __init__(self):
        super().__init__()
        ...
        # Set mặc định file lưu account edu
        if not self.dataFile.text().strip():
            self.dataFile.setText("edu_accounts.txt")
        
        # Set title cho window
        self.setWindowTitle("Tool Đăng Ký Email EDU")
    ...
    def start_thread(self):
        ...
        if not token:
            ...
            return
        if not data_file:
            data_file = "edu_accounts.txt"  # Set mặc định nếu không có file được chọn
        ...
        self.thread = WorkerThreadEdu(config)
