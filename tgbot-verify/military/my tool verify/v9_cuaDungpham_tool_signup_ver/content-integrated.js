...
let MAX_CONSECUTIVE_LIMIT = 5; // Stop after this many consecutive limit errors (can be changed by user)
let isReadingEmail = false; // Guard to prevent concurrent email reading

// Load consecutiveLimitErrors and MAX_CONSECUTIVE_LIMIT from storage on startup
chrome.storage.local.get(['veterans-consecutive-limit-errors', 'veterans-max-limit-errors'], (result) => {
    consecutiveLimitErrors = result['veterans-consecutive-limit-errors'] || 0;
    if (result['veterans-max-limit-errors'] !== undefined) {
        MAX_CONSECUTIVE_LIMIT = result['veterans-max-limit-errors'];
    }
    console.log('📊 Loaded consecutiveLimitErrors:', consecutiveLimitErrors, '/ Max:', MAX_CONSECUTIVE_LIMIT);
});

// Listen for changes to MAX_CONSECUTIVE_LIMIT and consecutiveLimitErrors settings
chrome.storage.onChanged.addListener((changes, areaName) => {
    if (areaName === 'local') {
        ...
        if (changes['veterans-consecutive-limit-errors']) {
            const newValue = changes['veterans-consecutive-limit-errors'].newValue;
            if (newValue !== undefined && newValue !== consecutiveLimitErrors) {
                consecutiveLimitErrors = newValue;
                console.log('📊 Synced consecutiveLimitErrors:', consecutiveLimitErrors);
            }
        }
    }
});
        ...
        chrome.storage.local.get(['veterans-data-lock'], (lockResult) => {
            ...
            chrome.storage.local.set({ 'veterans-data-lock': true }, () => {
                ...
                const storageUpdate = {
                    ...
                    'veterans-data-list-file': updatedDataList,  // Also update file backup - remove processed veteran
                    'veterans-current-index': currentDataIndex
                };
                ...
                chrome.storage.local.set(
                    ...
                    () => {
                        ...
                        // Log backup update
                        console.log('💾 Also removed from file backup (veterans-data-list-file)');
                    }
                );
            });
        });
