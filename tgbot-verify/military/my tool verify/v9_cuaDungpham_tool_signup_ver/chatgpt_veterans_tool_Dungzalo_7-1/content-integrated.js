...
let chatgptAccountsArray = []; // Array of all ChatGPT accounts
let chatgptAccountIndex = 0; // Current account index
    ...
    if (message.action === 'startVerification' || message.action === 'startSignup') {
        try {
            ...
            if (message.account && Array.isArray(message.data)) {
                ...
                // Load accounts array if available
                if (message.accountsArray) {
                    chatgptAccountsArray = message.accountsArray;
                    chatgptAccountIndex = message.accountIndex || 0;
                }
            ...
            currentDataIndex = 0;
            stats = { processed: 0, success: 0, notApproved: 0, limited: 0 };
            isRunning = true;
            currentEmail = '';
            mailRetryCount = 0;
...
async function startSignupLoop() {
    ...
    try {
        ...
        currentDataIndex++;
        stats.processed++;
        stats.notApproved++; // Signup error counts as notApproved
        updateStats();
        chrome.storage.local.set({
...
async function handleOTPVerification(data) {
    ...
    const MAX_OTP_RETRIES = 10;
    let otpRetryCount = 0;
    let otpCode = null;

    while (otpRetryCount < MAX_OTP_RETRIES && !otpCode && isRunning) {
        try {
            ...
            const EMAIL_API_URL = 'https://buonqua.online/api/get_messages_oauth2.php';
            ...
            if (otpRetryCount > 0) {
                sendStatus(`📡 Retrying to read email from API... (${otpRetryCount}/${MAX_OTP_RETRIES})`, 'info');
                await delay(3000); // Đợi 3 giây trước khi retry
            } else {
            ...
            if (messages.length === 0) {
                ...
                if (otpRetryCount < MAX_OTP_RETRIES) {
                    ...
                    throw new Error('No emails found after ' + MAX_OTP_RETRIES + ' attempts');
                }
            }
        ...
        if (!otpCode) {
            ...
            if (!otpCode) {
                ...
                if (otpRetryCount < MAX_OTP_RETRIES) {
                    ...
                    throw new Error('6-digit OTP code not found in email after ' + MAX_OTP_RETRIES + ' attempts. Subject: ' + (foundEmail?.subject || 'N/A'));
                }
            }
            ...
            otpRetryCount++;
            if (otpRetryCount >= MAX_OTP_RETRIES) {
                // Quá 10 lần, xử lý lỗi
                console.error('❌ Failed to get OTP after ' + MAX_OTP_RETRIES + ' attempts:', error);
                
                // Clear cookie và chuyển sang account tiếp theo
                await handleOTPFailure(data);
                return; // Exit function
            }
            // Nếu chưa quá 10 lần, tiếp tục retry
            sendStatus(`⚠️ Error getting OTP, retrying... (${otpRetryCount}/${MAX_OTP_RETRIES}): ` + error.message, 'info');
            await delay(3000);
            continue;
        }
    }

    // Kiểm tra lại isRunning trước khi tiếp tục
    if (!isRunning || !otpCode) {
        return;
    }
...
// Handle OTP failure - clear cookie and move to next account
async function handleOTPFailure(failedAccount) {
    sendStatus('❌ Failed to get OTP after multiple attempts. Moving to next account...', 'error');
    
    // Clear cookies first
    await clearCookies();
    if (!isRunning) return;
    
    // Load accounts array from storage
    chrome.storage.local.get(['chatgpt-accounts-array', 'chatgpt-account-index'], async (result) => {
        chatgptAccountsArray = result['chatgpt-accounts-array'] || [];
        chatgptAccountIndex = result['chatgpt-account-index'] || 0;
        
        if (chatgptAccountsArray.length === 0 || chatgptAccountIndex + 1 >= chatgptAccountsArray.length) {
            // No more accounts, stop
            sendStatus('❌ All accounts processed! (Some may have failed OTP)', 'error');
            isRunning = false;
            chrome.storage.local.set({ 'veterans-is-running': false });
            updateUIOnStop();
            return;
        ...
        // Move to next account (skip failed one)
        chatgptAccountIndex++;
        chatgptAccount = chatgptAccountsArray[chatgptAccountIndex];
        
        // Reset state for new account
        chatgptAccount.signupCompleted = false;
        currentDataIndex = 0;
        currentEmail = '';
        mailRetryCount = 0;
        ...
        chrome.storage.local.set({
            'chatgpt-account': chatgptAccount,
            'chatgpt-account-index': chatgptAccountIndex,
            'veterans-current-index': 0,
            'veterans-is-running': true
        });
        
        sendStatus(`🔄 Skipping failed account, switching to account ${chatgptAccountIndex + 1}/${chatgptAccountsArray.length}: ${chatgptAccount.email}`, 'info');
        await delay(3000);
        
        if (!isRunning) return;
        
        // Navigate to ChatGPT and start signup again
        window.location.href = 'https://chatgpt.com';
        await delay(5000);
        
        if (!isRunning) return;
        
        // Start signup loop with new account
        await startSignupLoop();
    });
}
    ...
    try {
        ...
        if (!heading || !headingText) {
            ...
            if (bodyText.includes('Not approved') ||
                ...
                const isNotApproved = bodyText.includes('Not approved');
                const isLimited = bodyText.includes('Verification Limit Exceeded') || bodyText.includes('limit exceeded');
                const errorType = isNotApproved ? 'Not approved' :
                                 isLimited ? 'Verification Limit Exceeded' :
                                 'Error';
                ...
                if (isNotApproved) {
                    ...
                    // For other errors, count as notApproved
                    stats.notApproved++;
                }
                updateStats();
                chrome.storage.local.set({
            ...
            if (bodyText.includes("You've been verified") || bodyText.includes('verified')) {
                ...
                // Clear cookies and move to next account
        ...
        if (headingText.includes('Unlock this Military-Only Offer')) {
            ...
            removeProcessedData();
            stats.processed++;
            stats.notApproved++; // Generic error counts as notApproved
            updateStats();
...
async function readMailAndVerify() {
    ...
    try {
        ...
        if (verificationLink) {
            ...
            // Quét trang và kiểm tra kết quả sau khi bấm link xác minh
            ...
            // Kiểm tra Not Approved
            if (bodyText.includes('Not approved') || pageHTML.includes('Not approved')) {
                console.log('❌ Not Approved detected after clicking verification link');
                sendStatus('❌ Not Approved detected, moving to next data...', 'info');
            ...
            // Kiểm tra Limited Exceeded
            if (bodyText.includes('Verification Limit Exceeded') || 
                bodyText.includes('limit exceeded') ||
                pageHTML.includes('Verification Limit Exceeded') ||
                pageHTML.includes('limit exceeded')) {
                console.log('❌ Limited Exceeded detected after clicking verification link');
                sendStatus('❌ Limited Exceeded detected, moving to next data...', 'info');
            ...
            // Nếu không có lỗi, tiếp tục với checkAndFillForm
            await checkAndFillForm();
        } else {
            if (!isRunning) return;
...
async function clearCookies() {
    try {
        sendStatus('🍪 Clearing cookies...', 'info');
        ...
        if (cookiesToDelete.length === 0) {
            sendStatus('ℹ️ No cookies to clear', 'info');
            return;
        }
        ...
        sendStatus(`✅ Cleared ${deletedCount} cookies`, 'success');
        await delay(2000);
    } catch (error) {
        console.error('Error clearing cookies:', error);
        sendStatus('⚠️ Error clearing cookies: ' + error.message, 'error');
        await delay(2000);
    }
}

// Handle verification success - save log and stop process
async function handleVerificationSuccess() {
    try {
        // Lưu log email thành công với đầy đủ thông tin
        if (chatgptAccount && chatgptAccount.email) {
            chrome.storage.local.get(['successful-emails-log'], (result) => {
                const successLog = result['successful-emails-log'] || [];
                const currentData = dataArray[currentDataIndex];
                
                const emailInfo = {
                    // Account ChatGPT (có thể thiếu password nếu user tự đăng nhập)
                    emailChatgpt: chatgptAccount.email || '',
                    passwordChatgpt: chatgptAccount.password || '',
                    
                    // Time verified
                    timestamp: new Date().toISOString(),
                    
                    // Email info (có thể thiếu nếu user tự đăng nhập)
                    emailLogin: chatgptAccount.emailLogin || '',
                    passEmail: chatgptAccount.passEmail || '',
                    refreshToken: chatgptAccount.refreshToken || '',
                    clientId: chatgptAccount.clientId || '',
                    
                    // Data veteran (full data) - có thể thiếu nếu không có data
                    veteranData: currentData ? currentData.original : ''
                };
                
                // Chỉ thêm nếu chưa có trong log (check theo email-chatgpt)
                if (!successLog.find(item => item.emailChatgpt === emailInfo.emailChatgpt)) {
                    successLog.push(emailInfo);
                    chrome.storage.local.set({ 'successful-emails-log': successLog }, () => {
                        console.log('✅ Saved successful email to log:', emailInfo.emailChatgpt);
                        sendStatus(`✅ Saved success log: ${emailInfo.emailChatgpt}`, 'success');
                    });
                }
            });
        }
        
        // Dừng toàn bộ tiến trình, giữ nguyên trang verify thành công
        sendStatus('✅ Verification successful! Process stopped. Page remains unchanged.', 'success');
        isRunning = false;
        chrome.storage.local.set({ 'veterans-is-running': false });
        updateUIOnStop();
