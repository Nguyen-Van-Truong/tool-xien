    ...
    chrome.storage.local.get(
        ...
        (result) => {
            if (
                ...
                // Check current URL to decide which loop to start
                const currentUrl = window.location.href;
                setTimeout(() => {
                    if (currentUrl.includes('services.sheerid.com')) {
                        // On SheerID page - continue verification
                        console.log('📍 AutoResume: On SheerID page, starting verification loop...');
                        startVerificationLoop();
                    } else if (currentUrl.includes('chatgpt.com/veterans-claim')) {
                        // On veterans-claim page - start verification (click button)
                        console.log('📍 AutoResume: On veterans-claim page, starting verification loop...');
                        startVerificationLoop();
                    } else if (currentUrl.includes('auth.openai.com') ||
                        (currentUrl.includes('chatgpt.com') && !currentUrl.includes('veterans-claim'))) {
                        // On signup/auth page - continue signup
                        console.log('📍 AutoResume: On auth/signup page, starting signup loop...');
                        startSignupLoop();
                    } else {
                        // Unknown page - start signup loop to navigate to correct page
                        console.log('📍 AutoResume: Unknown page, starting signup loop...');
                        startSignupLoop();
                    }
                }, 2000);
            }
...
async function startSignupLoop() {
    ...
    try {
        ...
        if (hasEmailInput) {
            ...
            // Check if user is logged in by looking for chat interface elements
            const hasSignUpButton = Array.from(document.querySelectorAll('button, a')).some(el =>
                (el.innerText || el.textContent || '').toLowerCase().includes('sign up'));
            const isLoggedIn = document.body.innerText && (
                document.body.innerText.includes('New chat') ||
...
async function clickSignUpButton() {
    ...
    try {
        ...
        // Cách 2: Tìm theo href selector (no :contains - not valid CSS)
        if (!signUpButton) {
            const selectors = [
                '[href*="signup"]',
                '[href*="register"]',
