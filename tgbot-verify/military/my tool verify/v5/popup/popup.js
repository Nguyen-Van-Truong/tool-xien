// Popup JavaScript - Controls the extension popup UI
...
class PopupController {
    ...
    async init() {
        ...
        // Debug mode toggle
        this.debugMode.addEventListener('change', async (e) => {
            await chrome.storage.local.set({ debugMode: e.target.checked });
            this.addLog(`Debug mode: ${e.target.checked ? 'ON' : 'OFF'}`, 'info');
        });

        // AUTO-SAVE inputs on change (persist when popup closes)
