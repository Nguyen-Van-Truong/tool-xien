...
function setupPanelHandlers() {
    ...
    // Save ChatGPT Account button
    const chatgptSaveBtn = document.getElementById('chatgpt-account-save-btn');
    if (chatgptSaveBtn) {
        chatgptSaveBtn.addEventListener('click', () => {
            chrome.storage.local.get(['chatgpt-account-list'], (result) => {
                let accountList = result['chatgpt-account-list'];
                
                if (!accountList || !accountList.trim()) {
                    updateUIPanelStatus('❌ No account to save', 'error');
                    return;
                }
                
                const blob = new Blob([accountList], { type: 'text/plain' });
                const url = URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = 'chatgpt-accounts.txt';
                a.click();
                URL.revokeObjectURL(url);
                updateUIPanelStatus('✅ Account file saved successfully', 'success');
            });
        });
    }
    ...
    // Save Veterans Data button
    const veteransSaveBtn = document.getElementById('veterans-data-save-btn');
    if (veteransSaveBtn) {
        veteransSaveBtn.addEventListener('click', () => {
            chrome.storage.local.get(['veterans-data-list'], (result) => {
                ...
                const a = document.createElement('a');
                a.href = url;
                a.download = 'veterans-data.txt';
                a.click();
                URL.revokeObjectURL(url);
                updateUIPanelStatus('✅ Data file saved successfully', 'success');
            });
        });
    }
            ...
            chrome.storage.local.get(['chatgpt-account', 'veterans-data-list'], async (result) => {
                ...
                chrome.storage.local.set({
                    ...
                    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
                        if (tabs[0] && tabs[0].url && (tabs[0].url.includes('chatgpt.com') || tabs[0].url.includes('services.sheerid.com') || tabs[0].url.includes('auth.openai.com'))) {
