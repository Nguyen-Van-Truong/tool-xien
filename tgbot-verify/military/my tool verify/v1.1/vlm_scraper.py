#!/usr/bin/env python3
"""
🎖️ VLM API Scraper V1.1 - All Veterans (Multi-threaded)
Lấy tất cả veterans, với option start/end page và auto-save
Hỗ trợ multi-threading để tăng tốc
"""
...
def fetch_veterans(last_name: str, page: int = 1, dob_from: str = None) -> list:
    """Fetch veterans from API"""
    ...
    # Add birth year filter if specified
    if dob_from:
    ...
    try:
        with httpx.Client(timeout=60.0) as client:
            ...
            if response.status_code == 200:
                ...
                return []  # Return empty on error, don't print (too noisy in multi-thread)
    except:
        return []
...
def scrape_letter(letter: str, start_page: int, end_page: int, output_file: str, dob_from: str = None) -> int:
    ...
    for page in range(start_page, end_page + 1):
        raw_data = fetch_veterans(letter, page, dob_from)
        
        if not raw_data:
            consecutive_empty += 1
            if consecutive_empty >= 5:  # Stop after 5 consecutive empty pages
                break
            continue
...
def main():
    parser = argparse.ArgumentParser(description="VLM API Scraper V1.1 - All Veterans (Multi-threaded)")
    ...
    # Format dob_from as YYYY-01-01 if year is provided
    dob_from = f"{args.dob}-01-01" if args.dob else None
    
    print("=" * 60)
    print("🎖️  VLM API SCRAPER V1.1 - MULTI-THREADED")
    print("=" * 60)
    ...
    print(f"\n🚀 Starting {len(letters)} letters with {args.threads} threads...\n")
